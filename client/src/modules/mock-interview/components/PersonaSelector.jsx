...
const PersonaSelector = ({ selectedPersona, onSelect }) => {
  return (
    <div className="group relative rounded-3xl bg-white dark:bg-surface p-6 sm:p-8 border border-border shadow-[0_20px_40px_rgba(0,0,0,0.04)] dark:shadow-none backdrop-blur-xl transition-all duration-300 hover:shadow-[0_20px_50px_rgba(168,85,247,0.1)] hover:border-purple-500/30">
            ...
            return (
              <div
                ...
                className={`relative flex items-start gap-4 p-4 rounded-2xl cursor-pointer transition-all duration-300 border ${
                  isSelected 
                    ? `bg-gray-50 dark:bg-slate-800/80 ${persona.borderActive} scale-[1.02]`
                    : "bg-white dark:bg-surface border-border hover:bg-gray-50 hover:dark:bg-slate-800/50 hover:scale-[1.01]"
                }`}
              >
                ...
                <div className="flex-1">
                  <h4 className={`font-bold text-sm mb-1 transition-colors ${isSelected ? "text-text-main" : "text-text-muted hover:text-text-main"}`}>
                    {persona.name}
                  </h4>
                  <p className={`text-[11px] leading-snug transition-colors ${isSelected ? "text-text-main font-medium" : "text-text-muted"}`}>
                    {persona.description}
                  </p>
                </div>
